...
Treat `app-modules/vendra-affiliate` as the source of affiliate domain behavior and Filament admin UI.
...
Follow the existing `Affiliate` and `AffiliateUser` patterns for new affiliate entities.
...
- Run module checks from the package when possible: `composer --working-dir=app-modules/vendra-affiliate test` and `composer --working-dir=app-modules/vendra-affiliate analyse`.
